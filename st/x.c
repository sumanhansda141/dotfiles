...
void previewimage(const Arg *arg) {
  ...
  if (g.mode & ATTR_IMAGE) {
    ...
    fprintf(stderr, "Clicked on placeholder %u/%u, x=%d, y=%d\n", image_id,
            tgetimgplacementid(&g), tgetimgcol(&g), tgetimgrow(&g));
    gr_preview_image(image_id, arg->s);
  }
}

void showimageinfo(const Arg *arg) {
  Glyph g = getglyphat(mouse_col, mouse_row);
  if (g.mode & ATTR_IMAGE) {
    uint32_t image_id = tgetimgid(&g);
    fprintf(stderr, "Clicked on placeholder %u/%u, x=%d, y=%d\n", image_id,
    ...
    gr_show_image_info(image_id, tgetimgplacementid(&g), tgetimgcol(&g),
                       tgetimgrow(&g), tgetisclassicplaceholder(&g),
                       tgetimgdiacriticcount(&g), argv0);
  }
}

void togglegrdebug(const Arg *arg) {
  graphics_debug_mode = (graphics_debug_mode + 1) % 3;
  redraw();
}

void dumpgrstate(const Arg *arg) { gr_dump_state(); }

void unloadimages(const Arg *arg) { gr_unload_images_to_reduce_ram(); }

void toggleimages(const Arg *arg) {
...
static void xdrawunderdashed(Draw draw, Color *color, int x, int y, int w,
  ...
  for (int i = x - x % wavelen; i < x + w; i += wavelen) {
    ...
    if (startx < endx)
      XftDrawRect(xw.draw, color, startx, y, endx - startx, thick);
  }
}

/* Draws an undercurl. `h` is the total height, including line thickness. */
static void xdrawundercurl(Draw draw, Color *color, int x, int y, int w, int h,
                           int thick) {
  ...
  GC gc =
      XCreateGC(xw.dpy, XftDrawDrawable(xw.draw),
                GCForeground | GCLineWidth | GCLineStyle | GCCapStyle, &gcvals);
...
void xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len,
  ...
  if (dmode & DRAW_FG) {
    ...
    if (base.mode & ATTR_UNDERLINE || base.mode & ATTR_URL) {
      ...
      int liney = winy + win.cyo + dc.font.ascent + gap;
      /* Adjust liney to guarantee that a single underline fits. */
      liney -= MAX(0, liney + thick - (winy + win.cyo + win.ch));
      if (style == UNDERLINE_DOUBLE) {
        liney -= MAX(0, liney + doubleh - (winy + win.cyo + win.ch));
        XftDrawRect(xw.draw, &decor, winx, liney, width, thick);
        XftDrawRect(xw.draw, &decor, winx, liney + doubleh - thick, width,
                    thick);
      } else if (style == UNDERLINE_DOTTED) {
        xdrawunderdashed(xw.draw, &decor, winx, liney, width, thick * 2, 0.5,
                         thick);
      } else if (style == UNDERLINE_DASHED) {
        int wavelen = MAX(2, win.cw * 0.9);
        xdrawunderdashed(xw.draw, &decor, winx, liney, width, wavelen, 0.65,
                         thick);
      } else if (style == UNDERLINE_CURLY) {
        liney -= MAX(0, liney + curlh - (winy + win.cyo + win.ch));
        xdrawundercurl(xw.draw, &decor, winx, liney, width, curlh, thick);
      }
    }

    if (base.mode & ATTR_BOXDRAW) {
      drawboxes(winx, winy + win.cyo, width / len, win.ch, fg, bg, specs, len);
    } else {
      /* Render the glyphs. */
...
void xdrawimages(Glyph base, Line line, int x1, int y1, int x2) {
  ...
  if (x1 > 0 && (prev->mode & ATTR_IMAGE) &&
      (prev->fg & 0xFFFFFF) == image_id_24bits && prev->decor == base.decor) {
  ...
  for (int x = x1; x < x2; ++x) {
    ...
    // next one.
    if (last_col && (num_diacritics <= 1 || !cur_col) && cur_row == last_row)
      cur_col = last_col + 1;
    // If the additional id byte is not specified and the
    // coordinates are consecutive, assume the byte is also the
    // same.
    if (last_id_4thbyteplus1 && (num_diacritics <= 2 || !cur_id_4thbyteplus1) &&
    ...
    if (cur_col != last_col + 1 || cur_row != last_row ||
      ...
      if (last_row != 0) {
        ...
        gr_append_imagerect(xw.buf, image_id, placement_id, last_start_col - 1,
                            last_col, last_row - 1, last_row, last_start_x, y1,
...
void xfinishimagedraw() { gr_finish_drawing(xw.buf); }
